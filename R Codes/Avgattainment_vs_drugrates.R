...
school_drug_data = grouped_school_dataset %>% 
  left_join(grouped_drug_crime ,by=c("Town"="Town")) %>% 
...
ggplot(school_drug_data,aes(x=`Drug Offence Rate`,y= `Attainment Score`)) +
  ...
  geom_point(data = filter(school_drug_data,County=="Surrey"), aes(color=c("Blue"="Surrey"))) + #setting color as blye for Surrey's data point
