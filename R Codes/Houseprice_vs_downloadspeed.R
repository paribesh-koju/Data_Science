...
house_price_broadband_data = grouped_house_prices %>% 
  left_join(grouped_broadband_speeds,by="Town")

#creating a linear model 
l_model = lm(data=house_price_broadband_data, Price~`Average download speed (Mbit/s)`) 
...
ggplot(house_price_broadband_data,aes(x=`Average download speed (Mbit/s)`,y=Price)) +
  ...
  geom_point(data = filter(house_price_broadband_data,County.x=="SURREY"), aes(color=c("Blue"="Surrey"))) + #setting color as blye for Surrey's data point
